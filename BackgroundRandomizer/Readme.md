...
3. Compile o projeto (exemplo com MinGW/g++):
   ```bash
   g++ main.cpp -o wallpaperRandomizer.exe -std=c++17 -luser32 -lgdi32
   ```

   Ou, se estiver usando MSVC (Visual Studio), crie um projeto de Console App e adicione o `main.cpp` normalmente — as bibliotecas `user32.lib` e `gdi32.lib` já vêm linkadas por padrão em projetos Windows.
